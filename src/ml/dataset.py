...
class EditDataset(Dataset):
    ...
    Examples:
        ...
        >>> from src.embedding.small_molecule.edit_embedder import edit_embedder_morgan
        >>>
        >>> # Pre-compute once
        >>> edit_emb = edit_embedder_morgan(radius=2, n_bits=2048)
        >>> X = edit_emb.encode_from_pair_df(pairs_df)  # (n_pairs, 2048)
        >>> y = pairs_df['delta'].values
        >>>
        >>> # Use pre-computed embeddings
        >>> dataset = EditDataset(edit_embeddings=X, targets=y)
        >>>
        >>> # Option 2: On-the-fly embedding (for prototyping)
        >>> dataset = EditDataset(
        ...     pairs_df=pairs_df,
        ...     targets=pairs_df['delta'].values,
        ...     edit_embedder=edit_emb
        ... )
    """
...
def create_datasets_from_embeddings(
    ...
    Example:
        ...
        >>> edit_vecs = edit_emb.encode_from_pair_df(pairs_df)
        >>> targets = pairs_df['delta'].values
        >>>
        >>> # Split
        >>> X_train, X_val, y_train, y_val = train_test_split(
        ...     edit_vecs, targets, test_size=0.2
        ... )
        >>>
        >>> # Create datasets
        >>> train_dataset, val_dataset = create_datasets_from_embeddings(
